...
class DDPMScheduler:
    def __init__(
        ...
        self.betas = torch.linspace(
            beta_start ** 0.5, beta_end ** 0.5, train_timesteps, dtype=torch.float32) ** 2
    ...
    def _get_variance(self, timestep: int) -> torch.Tensor:
        ...
        alpha_cumprod_t_prev = self.alphas_cumprod[prev_t] if prev_t >= 0 else self.one_val
    ...
    def step(
        ...
        pred_original_sample = (
            latents - beta_cumprod_t ** 0.5 * model_output) / alpha_cumprod_t ** 0.5

        # Compute coefficients
        pred_original_sample_coeff = (
            alpha_cumprod_t_prev ** 0.5 * beta_t) / beta_cumprod_t
